...
use modules\bulletin\common\models\Bulletin;
    use modules\client\backend\models\BulletinSearch;
...
class DefaultController extends Controller
{
    ...
    */
    public function actionCreate()
    {
        ...
        $attributeTypeManager = new AttributeTypeManager();
        $dynamicModel = $attributeTypeManager->model;

        if ($model->load(Yii::$app->request->post()) && $dynamicModel->load(Yii::$app->request->post())) {
            $attributeValModels = [];
            foreach($dynamicModel->attributes as $key => $value) {
                $attributeValModels[] = new AttributeVal([
                  'attribute_id' => $attributeTypeManager->getIdByName($key),
                  'val' => $value,
                ]);
            }
...
//            self::loadMultiple($categoryAttributes, $data);
            $model->populateRelation('attributeVals', $attributeValModels);
            if($model->save()) {
