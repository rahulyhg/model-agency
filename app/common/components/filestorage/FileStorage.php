...
class FileStorage extends \yii\base\Component implements IFileStorage
{
  ...
  /**
   * @var string URL to storage folder. if useFileController === false storageUrl is required!
   */
  public $storageUrl;
  ...
  /**
   * @var bool if false a direct link to the file on the server will be used
   */
  public $useFileController = true;
  ...
  public function getFileUrl($id)
  {
    $fileModel = File::findOne($id);
    if( $fileModel ) {
      if($this->useFileController) {
        return Url::to([$this->fileRoute, 'name' => $fileModel->path]);
      } else {
        return $this->storageUrl . '/' . $this->_correctUrl($fileModel->path);
      }
    }
    return false;
  }
  ...
  private function _correctUrl(string $url)
  {
